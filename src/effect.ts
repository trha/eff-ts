...
export { Effect, decompose, runPure, handle, interpret, interpretWithCont, interpose, introEff1, introEff2 };
...
function interpret<Effs, Eff, R>(
  ...
  return handle(reqType, function* (r) { return r; }, function* (req, resume) {
...
function interpretWithCont<Effs, Eff, R>(
  reqType: Constructor<Eff>,
  interpreter: (req: Eff, resume: (value: any) => Effect<Exclude<Effs, Eff>, R>) => Effect<Exclude<Effs, Eff>, R>,
  eff: Effect<Effs | Eff, R>,
): Effect<Exclude<Effs, Eff>, R> {
  return handle(reqType, function* (r) { return r; }, interpreter, eff);
}

function handle<Effs, Eff, R1, R2>(
  reqType: Constructor<Eff>,
  handlePure: (ret: R1) => Effect<Exclude<Effs, Eff>, R2>,
  handleReq: (req: Eff, resume: (value: any) => Effect<Exclude<Effs, Eff>, R2>) => Effect<Exclude<Effs, Eff>, R2>,
...
function interpose<Effs, Eff, R1, R2>(
  ...
  handleReq: (req: Eff, resume: (value: any) => Effect<Effs | Eff, R2>) => Effect<Effs | Eff, R2>,
  eff: Effect<Effs | Eff, R1>
): Effect<Effs | Eff, R2> {
  let req = eff.next();
...
function introEff2<Effs, E1, E2, T>(eff: Effect<Effs, T>): Effect<Effs | E1 | E2, T> {
  return eff;
}
